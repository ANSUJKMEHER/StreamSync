...
function CanvasPanel() {
  ...
  useEffect(() => {
    ...
    import('../../services/yjsService').then(({ yjsService }) => {
      import('yjs').then((Y) => {
        const ymap = yjsService.getCanvasMap(roomId);
        ...
        if (!ymap.get('shapesMap')) ymap.set('shapesMap', new Y.Map());
        if (!ymap.get('arrowsMap')) ymap.set('arrowsMap', new Y.Map());
        if (!ymap.get('autoPositionsMap')) ymap.set('autoPositionsMap', new Y.Map());

        // Initial load from Yjs
        const loadFromYjs = () => {
          const currentShapesMap = ymap.get('shapesMap') as Y.Map<Y.Map<any>>;
          const currentArrowsMap = ymap.get('arrowsMap') as Y.Map<Y.Map<any>>;
          const currentAutoPositionsMap = ymap.get('autoPositionsMap') as Y.Map<{x: number, y: number}>;

          if (!currentShapesMap || !currentArrowsMap || !currentAutoPositionsMap) return;

          const yShapes: CanvasShape[] = [];
          currentShapesMap.forEach((shapeYMap) => {
            yShapes.push(shapeYMap.toJSON() as CanvasShape);
          });

          const yArrows: CanvasArrow[] = [];
          currentArrowsMap.forEach((arrowYMap) => {
            yArrows.push(arrowYMap.toJSON() as CanvasArrow);
          });
          
          const positions = new Map<string, {x: number, y: number}>();
          currentAutoPositionsMap.forEach((pos, id) => {
        ...
        const unsubscribe = useCanvasStore.subscribe((state, prevState) => {
          ymap.doc?.transact(() => {
            ...
            prevState.shapes.filter(s => !s.isAutoGenerated).forEach((s) => {
              if (!currentShapeIds.has(s.id)) currentShapesMap.delete(s.id);
            });

            manualShapes.forEach((shape) => {
              let yShape = currentShapesMap.get(shape.id);
              if (!yShape) {
                yShape = new Y.Map();
                Object.entries(shape).forEach(([k, v]) => yShape!.set(k, v));
                currentShapesMap.set(shape.id, yShape);
              } else {
            ...
            prevState.arrows.filter(a => !a.isAutoGenerated).forEach((a) => {
              if (!currentArrowIds.has(a.id)) currentArrowsMap.delete(a.id);
            });

            manualArrows.forEach((arrow) => {
              let yArrow = currentArrowsMap.get(arrow.id);
              if (!yArrow) {
                yArrow = new Y.Map();
                Object.entries(arrow).forEach(([k, v]) => yArrow!.set(k, v));
                currentArrowsMap.set(arrow.id, yArrow);
              } else {
            ...
            autoShapes.forEach(shape => {
              ...
              if (!prevShape || prevShape.x !== shape.x || prevShape.y !== shape.y) {
                currentAutoPositionsMap.set(shape.id, { x: shape.x, y: shape.y });
              }
            });
          ...
          {arrows.map((arrow) => {
            ...
            return (
              <Arrow
                ...
                pointerLength={10}
                pointerWidth={10}
                fill={arrow.isAutoGenerated ? '#64748b' : '#d0bcff'}
                stroke={arrow.isAutoGenerated ? '#64748b' : '#d0bcff'}
