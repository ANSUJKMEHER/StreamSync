...
wsA.on('message', (data) => {
  ...
  if (msg.type !== 'ping') console.log(`[A] Received: ${msg.type}`, msg.type === 'error' ? msg.payload : '');
});

wsB.on('message', (data) => {
  const msg = JSON.parse(data.toString());
  if (msg.type !== 'ping') console.log(`[B] Received: ${msg.type}`, msg.type === 'error' ? msg.payload : '');
});
